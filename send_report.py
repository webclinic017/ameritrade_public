    ...
    for  index  in range(1,len(candles['open'])):
        if "call" in candles['action_type'][index]:
            if candles.index[index] > inferior_time_limit and candles.index[index] < superior_time_limit:
                dict_of_contract =test_market_options(symbol=symbol, contractType="CALL", toDate=find_next_friday(), low_rank = low_rank, high_rank = high_rank )
                if  dict_of_contract != None:
        ...
        if "put" in candles['action_type'][index]:
            if candles.index[index] > inferior_time_limit and candles.index[index] < superior_time_limit:
                dict_of_contract =   test_market_options(symbol=symbol, contractType="PUT", toDate=find_next_friday(), low_rank = low_rank, high_rank = high_rank )
                if dict_of_contract != None :
